...
{
    public class HotelsFilter : Filter
    {
    ...
    public class HotelsController : Controller
    {
        ...
        public HotelsController(IUnitOfWork unitOfWork, IWebHostEnvironment hostEnvironment)
        {
            _unitOfWork = unitOfWork;
            _hostEnvironment = hostEnvironment;
        }
        public async Task<IActionResult> Index(string searchstring, string locationstring, int? searchcategoryid, string subcategory,  int businessPage = 1)
        {
            ...
            //else
            {
                ...
                //    var hotels = await _unitOfWork.Hotels.GetAllAsync(s => s.OnlineBooking == filter.OnlineBooking.GetValueOrDefault());
                //    businesslist = hotels.Select(s => s.Business).Intersect(businesslist);
                //}
                //if (filter.MinPrice != null)
                //{
                //    var price = await _unitOfWork.Hotels.GetAllAsync(s => s.Minprice >= filter.MinPrice.GetValueOrDefault());
                //    businesslist = price.Select(s => s.Business).Intersect(businesslist);
                //}
                //if (filter.MaxPrice != null)
                //{
                //    var price = await _unitOfWork.Hotels.GetAllAsync(s => s.MinPrice <= filter.MaxPrice.GetValueOrDefault());
                //    businesslist = price.Select(s => s.Business).Intersect(businesslist);
                //}
            ...
            if (ModelState.IsValid)
            {
                ...
                if (categoryVM.HealthCare.Id == 0)
                {
                    ...
                    //if (result)
                    {
                        ...
                        //return LocalRedirect("/Customer/Hotels/AddressIndex?BusinessId=" + businessId);
        ...
        public async Task<IActionResult> AddressIndex(int BusinessId)
        {
            ...
            if (address.Count() == 0)
            {
                //return LocalRedirect("/Customer/Hotels/AddressUpsert?BusinessId=" + BusinessId);
        ...
        public async Task<IActionResult> AddressUpsert(ContactVM contactVM)
        {
            if (ModelState.IsValid)
            {
                ...
                //return LocalRedirect("/Customer/Hotels/Upsert?Id=" + contactVM.Business.Id);
