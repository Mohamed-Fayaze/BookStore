...
{
    public class FitnessFilter : Filter
    {
    ...
    public class FitnessController : Controller
    {
        ...
        public FitnessController(IUnitOfWork unitOfWork, IWebHostEnvironment hostEnvironment)
        {
            _unitOfWork = unitOfWork;
            _hostEnvironment = hostEnvironment;
        }
        public async Task<IActionResult> Index(string searchstring, string locationstring, int? searchcategoryid, string subcategory, FitnessFilter filter, int businessPage = 1)
        {
            ...
            //else
            {
                ...
                if (filter.OnlineBooking != null)
                {
                    ...
                    var fitness = await _unitOfWork.Fitness.GetAllAsync(s => s.OnlineBooking == filter.OnlineBooking.GetValueOrDefault());
                    businesslist = fitness.Select(s => s.Business).Intersect(businesslist);
                }
                if (filter.MinPrice != null)
                {
                    var price = await _unitOfWork.Fitness.GetAllAsync(s => s.minprice >= filter.MinPrice.GetValueOrDefault());
                    businesslist = price.Select(s => s.Business).Intersect(businesslist);
                }
                //if (filter.MaxPrice != null)
                //{
                //    var price = await _unitOfWork.Fitness.GetAllAsync(s => s.MinPrice <= filter.MaxPrice.GetValueOrDefault());
                //    businesslist = price.Select(s => s.Business).Intersect(businesslist);
                //}
            ...
            if (ModelState.IsValid)
            {
                ...
                if (categoryVM.HealthCare.Id == 0)
                {
                    ...
                    //if (result)
                    {
                        ...
                        //return LocalRedirect("/Customer/Fitness/AddressIndex?BusinessId=" + businessId);
        ...
        public async Task<IActionResult> AddressIndex(int BusinessId)
        {
            ...
            if (address.Count() == 0)
            {
                //return LocalRedirect("/Customer/Fitness/AddressUpsert?BusinessId=" + BusinessId);
        ...
        public async Task<IActionResult> AddressUpsert(ContactVM contactVM)
        {
            if (ModelState.IsValid)
            {
                ...
                //return LocalRedirect("/Customer/Fitness/Upsert?Id=" + contactVM.Business.Id);
