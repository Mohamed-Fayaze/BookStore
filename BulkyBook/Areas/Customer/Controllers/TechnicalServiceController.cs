...
{
    public class TechnicalServiceFilter : Filter
    {
    ...
    public class TechnicalServiceController : Controller
    {
        ...
        public TechnicalServiceController(IUnitOfWork unitOfWork, IWebHostEnvironment hostEnvironment)
        {
            _unitOfWork = unitOfWork;
            _hostEnvironment = hostEnvironment;
        }
        public async Task<IActionResult> Index(string searchstring, string locationstring, int? searchcategoryid, string subcategory, TechnicalServiceFilter filter, int businessPage = 1)
        {
            ...
            //else
            {
                ...
                if (filter.OnlineBooking != null)
                {
                    ...
                    var technicalServices = await _unitOfWork.TechnicalService.GetAllAsync(s => s.OnlineBooking == filter.OnlineBooking.GetValueOrDefault());
                    businesslist = technicalServices.Select(s => s.Business).Intersect(businesslist);
                }
                //if (filter.MinPrice != null)
                //{
                //    var price = await _unitOfWork.TechnicalService.GetAllAsync(s => s.Minprice >= filter.MinPrice.GetValueOrDefault());
                //    businesslist = price.Select(s => s.Business).Intersect(businesslist);
                //}
                //if (filter.MaxPrice != null)
                //{
                //    var price = await _unitOfWork.TechnicalService.GetAllAsync(s => s.MinPrice <= filter.MaxPrice.GetValueOrDefault());
                //    businesslist = price.Select(s => s.Business).Intersect(businesslist);
                //}
            ...
            if (ModelState.IsValid)
            {
                ...
                if (categoryVM.HealthCare.Id == 0)
                {
                    ...
                    //if (result)
                    {
                        ...
                        //return LocalRedirect("/Customer/TechnicalService/AddressIndex?BusinessId=" + businessId);
        ...
        public async Task<IActionResult> AddressIndex(int BusinessId)
        {
            ...
            if (address.Count() == 0)
            {
                //return LocalRedirect("/Customer/TechnicalService/AddressUpsert?BusinessId=" + BusinessId);
        ...
        public async Task<IActionResult> AddressUpsert(ContactVM contactVM)
        {
            if (ModelState.IsValid)
            {
                ...
                //return LocalRedirect("/Customer/TechnicalService/Upsert?Id=" + contactVM.Business.Id);
