...
{
    public class ProfessionalAssistanceFilter : Filter
    {
        public int? SubcategoryId { get; set; }
        public int? SpecialityId { get; set; }
    }
    [Area("Customer")]
    public class ProfessionalAssistanceController : Controller
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IWebHostEnvironment _hostEnvironment;
        private static string message;
        public ProfessionalAssistanceController(IUnitOfWork unitOfWork, IWebHostEnvironment hostEnvironment)
        {
            _unitOfWork = unitOfWork;
            _hostEnvironment = hostEnvironment;
        }
        public async Task<IActionResult> Index(string searchstring, string locationstring, int? searchcategoryid, string subcategory, ProfessionalAssistanceFilter filter, int businessPage = 1)
        {
            ...
            //else
            {
                ...
                if (filter.OnlineBooking != null)
                {
                    ...
                    var professionalAssistance = await _unitOfWork.ProfessionalAssistance.GetAllAsync(s => s.OnlineBooking == filter.OnlineBooking.GetValueOrDefault());
                    businesslist = professionalAssistance.Select(s => s.Business).Intersect(businesslist);
                }
                //if (filter.MinPrice != null)
                //{
                //    var price = await _unitOfWork.ProfessionalAssistance.GetAllAsync(s => s.Minprice >= filter.MinPrice.GetValueOrDefault());
                //    businesslist = price.Select(s => s.Business).Intersect(businesslist);
                //}
                //if (filter.MaxPrice != null)
                //{
                //    var price = await _unitOfWork.ProfessionalAssistance.GetAllAsync(s => s.MinPrice <= filter.MaxPrice.GetValueOrDefault());
                //    businesslist = price.Select(s => s.Business).Intersect(businesslist);
                //}
            ...
            if (ModelState.IsValid)
            {
                ...
                if (categoryVM.HealthCare.Id == 0)
                {
                    ...
                    //if (result)
                    {
                        ...
                        //return LocalRedirect("/Customer/ProfessionalAssistance/AddressIndex?BusinessId=" + businessId);
        ...
        public async Task<IActionResult> AddressIndex(int BusinessId)
        {
            ...
            if (address.Count() == 0)
            {
                //return LocalRedirect("/Customer/ProfessionalAssistance/AddressUpsert?BusinessId=" + BusinessId);
        ...
        public async Task<IActionResult> AddressUpsert(ContactVM contactVM)
        {
            if (ModelState.IsValid)
            {
                ...
                //return LocalRedirect("/Customer/ProfessionalAssistance/Upsert?Id=" + contactVM.Business.Id);
