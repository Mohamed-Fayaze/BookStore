...
{
    public class RetailFilter : Filter
    {
    ...
    public class RetailController : Controller
    {
        ...
        public RetailController(IUnitOfWork unitOfWork, IWebHostEnvironment hostEnvironment)
        {
            _unitOfWork = unitOfWork;
            _hostEnvironment = hostEnvironment;
        }
        public async Task<IActionResult> Index(string searchstring, string locationstring, int? searchcategoryid, string subcategory, RetailFilter filter, int businessPage = 1)
        {
            ...
            //else
            {
                ...
                if (filter.OnlineBooking != null)
                {
                    ...
                    var retail = await _unitOfWork.Retail.GetAllAsync(s => s.OnlineBooking == filter.OnlineBooking.GetValueOrDefault());
                    businesslist = retail.Select(s => s.Business).Intersect(businesslist);
                }
                //if (filter.MinPrice != null)
                //{
                //    var price = await _unitOfWork.Retail.GetAllAsync(s => s.Minprice >= filter.MinPrice.GetValueOrDefault());
                //    businesslist = price.Select(s => s.Business).Intersect(businesslist);
                //}
                //if (filter.MaxPrice != null)
                //{
                //    var price = await _unitOfWork.Retail.GetAllAsync(s => s.MinPrice <= filter.MaxPrice.GetValueOrDefault());
                //    businesslist = price.Select(s => s.Business).Intersect(businesslist);
                //}
        ...
        public async Task<IActionResult> Upsert(int? Id, int categoryid, int subcategoryid)
        {
            ...
            //categoryVM.HealthCare = await _unitOfWork.HealthCare.GetFirstOrDefaultAsync(s => s.BusinessId == Id);
            ...
            if (ModelState.IsValid)
            {
                ...
                if (categoryVM.HealthCare.Id == 0)
                {
                    ...
                    //if (result)
                    {
                        ...
                        //categoryVM.HealthCare.BusinessId = businessId;
                        ...
                        //return LocalRedirect("/Customer/Retail/AddressIndex?BusinessId=" + businessId);
        ...
        public async Task<IActionResult> AddressIndex(int BusinessId)
        {
            ...
            if (address.Count() == 0)
            {
                //return LocalRedirect("/Customer/Retail/AddressUpsert?BusinessId=" + BusinessId);
        ...
        public async Task<IActionResult> AddressUpsert(ContactVM contactVM)
        {
            if (ModelState.IsValid)
            {
                ...
                //return LocalRedirect("/Customer/Retail/Upsert?Id=" + contactVM.Business.Id);
