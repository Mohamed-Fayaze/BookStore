...
{
    public class RestaurantFilter : Filter
    {
    ...
    public class RestaurantController : Controller
    {
        ...
        public async Task<IActionResult> Index(string searchstring, string locationstring, int? searchcategoryid, string subcategory, RestaurantFilter filter, int businessPage = 1)
        {
            ...
            //else
            {
                ...
                if (filter.OnlineBooking != null)
                {
                    ...
                    var restaurant = await _unitOfWork.Restaurant.GetAllAsync(s => s.OnlineBooking == filter.OnlineBooking.GetValueOrDefault());
                    businesslist = restaurant.Select(s => s.Business).Intersect(businesslist);
                }
                //if (filter.MinPrice != null)
                //{
                //    var price = await _unitOfWork.Restaurant.GetAllAsync(s => s.Minprice >= filter.MinPrice.GetValueOrDefault());
                //    businesslist = price.Select(s => s.Business).Intersect(businesslist);
                //}
                //if (filter.MaxPrice != null)
                //{
                //    var price = await _unitOfWork.Restaurant.GetAllAsync(s => s.MinPrice <= filter.MaxPrice.GetValueOrDefault());
                //    businesslist = price.Select(s => s.Business).Intersect(businesslist);
                //}
            ...
            if (ModelState.IsValid)
            {
                ...
                if (categoryVM.HealthCare.Id == 0)
                {
                    ...
                    //if (result)
                    {
                        ...
                        //return LocalRedirect("/Customer/Restaurant/AddressIndex?BusinessId=" + businessId);
        ...
        public async Task<IActionResult> AddressIndex(int BusinessId)
        {
            ...
            if (address.Count() == 0)
            {
                //return LocalRedirect("/Customer/Restaurant/AddressUpsert?BusinessId=" + BusinessId);
        ...
        public async Task<IActionResult> AddressUpsert(ContactVM contactVM)
        {
            if (ModelState.IsValid)
            {
                ...
                //return LocalRedirect("/Customer/Restaurant/Upsert?Id=" + contactVM.Business.Id);
